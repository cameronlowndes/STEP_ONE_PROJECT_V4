...
const Login = ({ onLogin }) => {
    ...
    const handleTestLogin = () => {
        ...
        setError(''); // Clear any previous errors
        setSuccess('Successfully logged in with test account!'); // Set success message
        onLogin(username); // Call the onLogin prop with the username
        setTimeout(() => {
            navigate('/'); // Redirect to home page after a brief moment
        }, 1000); // Wait 1 second before redirecting
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r bg-gray-100">
            <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-lg">
                ...
                {success && <p className="text-green-500 text-center">{success}</p>} {/* Display success message if exists */}
