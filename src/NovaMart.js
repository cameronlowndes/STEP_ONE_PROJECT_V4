...
import { initialProducts } from './ShopPage'; // Import initial products from Shop
import { Cart } from './ShopPage'; // Import the Cart component
...
const NovaMart = () => {
    ...
    const addToCart = (product) => {
        setCart((prevCart) => [
            ...prevCart,
            { ...product, discountedPrice: (product.price * 0.80).toFixed(2) }
        ]);
    };
    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-100"> {/* Set background color */}
            <main className="flex-grow flex pt-20"> {/* Add padding-top for navbar height */}
                    ...
                    <section className="bg-gray-100 flex-grow p-8 flex flex-col items-center justify-center">
                        ...
                        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl">
                            {initialProducts.slice(0, 10).map(product => {
                                const discountedPrice = (product.price * 0.80).toFixed(2); // Calculate 20% discount

                                return (
                                    <li key={product.id} className="flex flex-col items-start p-6 border rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 bg-white">
                ...
                <div className="w-1/3 p-4"> {/* Adjust width as needed */}
                    <Cart 
                        ...
                        clearCart={clearCart} 
                        onCheckout={handleCheckout} // Ensure to pass the correct handler
                    />
                </div>
            </main>
